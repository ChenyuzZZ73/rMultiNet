...
#' @name       ProjectedGD
###Input:
#' @param      Ini_list list(adj_tensor,U_int, W_int, delta) delta for incoherent parameters: (delta_1,delta_2,delta_3)
#' @param      Cmax upper bound
#' @param      eta_outer learning rate eta
#' @param      tmax_outer number of iterations in gradient descent
#' @param      p_type type of link function included "logit", "probit" and "poisson"
#' @param      rd if use stochastic sampling ("rand" or "Non")
#' @param      scale_par scale parameter (sigma in the paper)
#' @param      sample_size
#' @import     Matrix
#' @import     rTensor
#' @import     glmnet
...
ProjectedGD <- function(Ini_list, Cmax=1, eta_outer = 1e-04,
                           tmax_outer = 35, p_type ="logit", rd ="Non", show = TRUE, sgma =1, sample_size =5000)
{
